...
class Reference
{
    ...
    protected function chargeValidator($data, &$charge_vars)
    {
        $charge_vars = array_merge($this->REQUIRED_CHARGE_VARS, $charge_vars);
                ...
                foreach ($SecondArray as $Secondkey => $req_status2) {
                    if (is_array($req_status2)) {
                        ...
                        foreach ($ThirdArray as $Thirdkey => $req_status3) {
                            if (isset($data[$Firstkey][$Secondkey]) && key_exists(
                                    $Thirdkey,
                                ...
                                if ($req_status3) {
                                    ...
                                    throw new \InvalidArgumentException(
                                        "InvalidArgumentException $Firstkey.$Secondkey.$Thirdkey required"
                                    );
                                } else {
                                    if (in_array(
                                            $Thirdkey,
                                            ['country_code', 'number']
                                        ) && $this->CHARGE_VARS[$Firstkey][$Secondkey][$Thirdkey] == null) {
                                        if (!isset($this->CHARGE_VARS['customer']['email']) || isset($this->CHARGE_VARS['customer']['email']) && $this->CHARGE_VARS['customer']['email'] == null) {
                                            throw new \InvalidArgumentException(
                                                "InvalidArgumentException $Firstkey.phone or $Firstkey.email is required"
                                            );
                                        }
                                    }
                                }
                        ...
                        if (isset($data[$Firstkey]) && key_exists($Secondkey, $data[$Firstkey])) {
                            ...
                            if ($req_status2) {
                                // missing required parm
                                throw new \InvalidArgumentException(
                                    "InvalidArgumentException $Firstkey.$Secondkey required"
                                );
                            }
                        }
                    }
        ...
        $charge_vars = array_merge($this->CHARGE_VARS, $charge_vars);
    }
