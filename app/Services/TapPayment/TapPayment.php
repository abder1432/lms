...
class   TapPayment extends Reference implements Tap
{
    ...
    public function setCardToken($token_id) {
        if (!empty($token_id)) {
            $this->CHARGE_VARS['source']['id'] = $token_id;
            //$this->CARD_SET = true;
        }
    }
    ...
    public function card($data)
    {
        ...
        if ($err) {
            ...
            if (isset($json_response->errors) && is_array($json_response->errors) && count(
    ...
    {
        $this->chargeValidator($data, $this->CHARGE_VARS);
        $this->CHARGE_VARS['save_card'] = $this->CHARGE_VARS['save_card'] ? "true" : "false";
        $this->CHARGE_VARS['receipt']['email'] = $this->CHARGE_VARS['receipt']['email'] ? "true" : "false";
        $this->CHARGE_VARS['receipt']['sms'] = $this->CHARGE_VARS['receipt']['sms'] ? "true" : "false";
        ...
        if ($this->CARD_SET) {
            curl_setopt_array(
                ...
                array(
                    ...
                    CURLOPT_HTTP_VERSION => CURL_HTTP_VERSION_1_1,
                    CURLOPT_CUSTOMREQUEST => "POST",
                    CURLOPT_POSTFIELDS => "{\"amount\":" . $this->CHARGE_VARS['amount'] . ",\"currency\":\"" . $this->CHARGE_VARS['currency'] . "\",\"threeDSecure\":" . $this->CHARGE_VARS['threeDSecure'] . ",\"save_card\":" . $this->CHARGE_VARS['save_card'] . ",\"description\":\"" . $this->CHARGE_VARS['description'] . "\",
        ...
        if ($err) {
            ...
            if (isset($json_response['object']) && $json_response['object'] == "charge") {
                if (isset($json_response['transaction']['url'])) {
                    return $json_response['transaction']['url'];
                }
