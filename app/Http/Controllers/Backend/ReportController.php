...
class ReportController extends Controller
{
    ...
    {
        return view('backend.reports.teachers');
    }
    ...
    public function getTeachersData(Request $request)
    {
        ...
        ->addColumn('sales', function ($q) {
            ...
            $course_sales = Order::query()
                ->with('items')
                ->where('status', '=', 1)
                ->whereHas('items', function ($q) use ($courses) {
                    $q->where('item_type', '=', Course::class)
                        ->whereIn('item_id', $courses);
                });
            return $course_sales->count() ?? 0;
        })
        ->addColumn('balance', function ($q) {
            ...
            return round($course_earnings = $course_earnings->sum('amount'), 2) ?? (float)0;
        })
        ->make();
    }
