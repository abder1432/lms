...
class CertificateController extends Controller
{
    ...
    public function generateCertificate(Request $request)
    {
        ...
        if (($course != null) && ($course->progress() == 100)) {
            ...
            $teacher = $course->teachers->first()->teacherProfile;

            $data = [
                'name' => auth()->user()->name,
                'course_name' => $course->title,
                'date' => Carbon::now()->format('d M, Y'),
                'color' => config('certificate_color') ?? null,
                'logo' => \Storage::url(config('certificate_logo')) ?? null,
                'background' => \Storage::url(config('certificate_background')) ?? null
            ];
