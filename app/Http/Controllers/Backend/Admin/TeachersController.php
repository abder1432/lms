...
class TeachersController extends Controller
{
    ...
    public function getData(Request $request)
    {
        ...
        return DataTables::of($teachers)
            ->addIndexColumn()
            ->addColumn('actions', function ($q) use ($has_view, $has_edit, $has_delete, $has_availability, $request) {
                $view = "";
                $edit = "";
                $delete = "";
                if ($request->show_deleted == 1) {
                ...
                if ($has_availability) {
                    $availability = view('backend.datatable.action-availability')
                        ->with(['route' => route('admin.teachers.availability', ['teacher' => $q->id])])
                        ->render();
                    $view .= $availability;
                }
...
//        $request = $this->saveFiles($request);
    ...
    public function availability(User $teacher)
    {
        return view('backend.teachers.availability', compact('teacher'));
    }
